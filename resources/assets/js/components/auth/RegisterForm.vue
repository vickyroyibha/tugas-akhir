<template>
 <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
  ...
  <div class="form-group has-feedback " :class="{ 'has-error': form.errors.has('username') }">
   <input type="text" class="form-control" :placeholder="trans('adminlte_lang_message.username')" name="username" value="" v-model="form.username" autofocus/>
   <span class="glyphicon glyphicon-user form-control-feedback"></span>
   <transition name="fade">
...
export default {
  ...
  data: function () {
    return {
      form: new Form({ name: '', username: '', password: '', password_confirmation: '', terms: '' })
    }
  },
  watch: {
